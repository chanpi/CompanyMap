...
@synthesize sortKeys = sortKeys_;
@synthesize humanReadable = humanReadable_;

// 互換性のために残してあるメソッド。使用しない。
...
- (NSString*)indent
{
    return [@"\n" stringByPaddingToLength:(1 + 2 * depth_) withString:@" " startingAtIndex:0];  // 1はおそらく改行の分
}
...
- (BOOL)appendValue:(id)fragment into:(NSMutableString*)json
{
    if ([fragment isKindOfClass:[NSDictionary class]]) {
        ...
        [self appendValue:[self proxyForJson] into:json];
    } else {
        [self addErrorWithCode:EUNSUPPORTED description:[NSString stringWithFormat:@"JSON serialization not supported for %@", [fragment class]]];
        return NO;
    }
    return YES;
}
